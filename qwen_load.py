...
def _broadcast_tensor(tensor, name, state_dict, mp_group, params_dtype) -> torch.Tensor:
    ...
    if tensor_shape is None:
        ...
        print(f"tensor:[{name}] not in state_dict, skip load")
        return
...
def _broadcast_tp_shard_tensor_qkv(tensor, q_name, k_name, v_name, state_dict, mp_group, params_dtype,
    ...
    if chunk_shape is None:
        # all or none ranks in the mp_group should reach here
        print(f"tp_shard tensor:[{q_name}] not in state_dict, skip loading")
        return
...
def _broadcast_tp_shard_tensor_qkv_bias(tensor, q_name, k_name, v_name, state_dict, mp_group, params_dtype,
    ...
    if torch.distributed.get_rank() == 0:
        assert (q_name in state_dict and k_name in state_dict and v_name in state_dict)
        full_weight_q_bias = state_dict[q_name]
        full_weight_k_bias = state_dict[k_name]
        full_weight_v_bias = state_dict[v_name]
...
def load_state_dict_to_megatron_qwen(state_dict, models, config, params_dtype, is_value_model=False):
    ...
    assert len(models) == virtual_pp_size
    num_layers_per_model = config.num_hidden_layers // pp_size // virtual_pp_size
    assert num_layers_per_model * pp_size * virtual_pp_size == config.num_hidden_layers
    ...
    if dp_rank == 0:
        ...
        for layer in range(config.num_hidden_layers):
            ...
            qwen_model_module = _get_qwen_model(models[dst_virtual_pp_rank])
            sync_layer = qwen_model_module.layers[dst_layer_idx]

            _broadcast_tensor(
                sync_layer.input_layernorm.weight if dst_pp_rank == pp_rank else None,
                f"{layer_name}.input_layernorm.weight",
                state_dict, mp_group, params_dtype
            )
            ...
            _broadcast_tp_shard_tensor_qkv(
                ...
                state_dict, mp_group, params_dtype, config
            )

            _broadcast_tp_shard_tensor_qkv_bias(
        ...
        qwen_model_module = _get_qwen_model(models[-1])
        _broadcast_tensor(
            getattr(qwen_model_module.final_norm, "weight", None),
            "model.norm.weight",
            state_dict, mp_group, params_dtype
        )
        ...
        if is_value_model:
            ...
            if 'lm_head.weight' in state_dict and state_dict['lm_head.weight'].shape[0] == 1:
                ...
                print('fail to match lm_head in value_model')
            # else:

            #     _broadcast_tensor(lm_head_weight, "lm_head.weight")

        else:
            _broadcast_tp_shard_tensor(lm_head_weight, "lm_head.weight", state_dict, mp_group, params_dtype)
    dist.barrier()
